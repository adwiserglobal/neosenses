...
export function WhatsAppButton({ message }: WhatsAppButtonProps) {
  ...
  return (
    <motion.a
      ...
      className="fixed bottom-6 right-6 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[var(--color-whatsapp)] text-[#0b2e18] shadow-lg transition-transform hover:scale-110 active:scale-95"
    ...
    >
      ...
      {/* Pulse ring */}
      <span className="absolute inset-0 animate-ping rounded-full bg-[var(--color-whatsapp)]/30" style={{ animationDuration: "2s" }} />
    </motion.a>
  );
}
